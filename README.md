...
- **폰트 파일 업로드**: woff2 파일을 직접 서버에 업로드하여 사용
...
**지원 형식**: woff2만 지원  
**최대 크기**: 10MB  
**저장 위치**: `SillyTavern/public/webfonts/`
...
1. **업로드 실패**
   - 파일 형식 확인 (woff2만 지원)
   - 파일 크기 확인 (10MB 이하)
   - SillyTavern 서버가 실행 중인지 확인
   - 파일 이름에 특수문자가 없는지 확인

2. **업로드한 폰트가 적용되지 않음**
   - `SillyTavern/public/webfonts` 폴더 권한 확인
   - 서버 재시작 후 다시 시도
   - 브라우저 캐시 삭제 및 새로고침
