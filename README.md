...
- 각 마크다운 요소별로 폰트, 크기, **배경색**, **배경 여백** 독립적으로 설정 가능
- 배경색 설정 시 자동으로 패딩, 테두리 반경 등 스타일 적용
- **배경 여백 조절**: 1~10px까지 자유롭게 조절 (기본값: 2px)
- **투명도 지원**: `rgba(20, 20, 30, 0.6)` 형식으로 투명도 설정 가능
- 다양한 색상 형식 지원: `#hex`, `rgb()`, `rgba()`
- **배경색 제거**: 입력 필드를 비우거나 `none`, `transparent` 입력 시 배경색 제거
